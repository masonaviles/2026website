...
const WORKSPACE: WorkspaceItem[] = [
  { label: "~/profile", href: "/", active: true, badge: { text: "main", tone: "accent" } },
        ...
        {WORKSPACE.map((item) => (
          <li key={item.label}>
            <Link
              ...
              style={
                item.active ? { background: "var(--rail-active)" } : undefined
              }
              aria-current={item.active ? "page" : undefined}
            >
              <span className="w-2.5 text-ink-mute">{item.active ? "▾" : "▸"}</span>
